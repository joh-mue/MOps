...
def execute_sfn(state_machine_name, matrix_names, block_size, split_size, mat_dimA, mat_dimB):
...
def invoke_matrix_multiplication(state_machine_name, executionName, name_matrixA, name_matrixB,
                                 block_size, split_size, mat_dimA, mat_dimB):
    """ execution_info.keys() :> ['deploy-nr', 'split-executions', 'split'] """
    sfn_input = create_input(state_machine_name, executionName, name_matrixA, name_matrixB, block_size, split_size, mat_dimA, mat_dimB)
...
def create_input(state_machine_name, executionName, name_matrixA, name_matrixB, block_size,
                 split_size, matrix_dimensionsA, matrix_dimensionsB):
    input = {
            ...
            'matA': {
                ...
                'rows': matrix_dimensionsA.height,
                'columns': matrix_dimensionsA.width
            },
            'matB': {
                'bucket': BUCKET,
                'folder': name_matrixB,
                'block-size': block_size,
                'rows': matrix_dimensionsB.height,
                'columns': matrix_dimensionsB.height
            },
            'result': {
                'bucket': BUCKET,
...
def create_plots_per_lambda(csv_path, state_machine_name, to_file=True):
    ...
    for l_type in ['intermediate', 'collect', 'accumulate']:
        ...
        if (any(map(lambda x: len(x) == 0, lambda_timings))):
            ...
            # with evarage plot_time_profile(with_average(lambda_timings), l_type, state_machine_name, os.path.dirname(csv_path), to_file, ybound=ybound, xbound=xbound)
...
def plot_time_profile(timings, lambda_type, state_machine_name, plot_dir=None, to_file=True, ybound=None, xbound=None):
    ...
    ax.set_xlabel('execution index')#, last one is average across executions')
...
if __name__ == "__main__":
    ...
    matrix_dimension_sets = [
            ...
            MatrixDimensions(height=6000, width=2000),
    ...
    for block_size in block_sizes:
        for matrix_dimensions in matrix_dimension_sets:
            ...
            matrix_dimensionsA = MatrixDimensions(height=6000, width=2000)
            matrix_dimensionsB = MatrixDimensions(height=2000, width=6000)

            matrix_nameA = '{}_{}kx{}k_bs{}k'.format(PREFIX, matrix_dimensionsA.height/1000, matrix_dimensionsA.width/1000, block_size/1000)
            matrix_nameB = '{}_{}kx{}k_bs{}k'.format(PREFIX, matrix_dimensionsB.height/1000, matrix_dimensionsB.width/1000, block_size/1000)
            matrix_names = [matrix_nameA, matrix_nameB]
            _log('Benchmarking {} * {} with blocksize={}'.format(matrix_nameA, matrix_nameB, block_size))

            create_matrix(matrix_nameA, matrix_dimensionsA, block_size)
            deploy_matrix(matrix_nameA, BUCKET)

            create_matrix(matrix_nameB, matrix_dimensionsB, block_size)
            deploy_matrix(matrix_nameB, BUCKET)
            ...
            # start execution
            state_machine_name = '{}-{}kx{}k'.format(SFN_PREFIX, matrix_dimensionsB.height/1000, matrix_dimensionsB.width/1000)
            if not replotting:
                split_executionARNs = execute_sfn(state_machine_name, matrix_names, block_size, split_size, matrix_dimensionsA, matrix_dimensionsB)
